...
namespace DataMasker.DataSources
{
    ...
    public class SqlDataSource : IDataSource
    {
        ...
        /// <returns></returns>
        /// <inheritdoc/>
        public async Task<IEnumerable<IDictionary<string, object>>> GetData(
        ...
        public async Task<int> GetCount(TableConfig config)
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                await connection.OpenAsync();
                ...
                TableConfig tableConfig)
        {
            ...
            sql += tableConfig.Columns.GetUpdateColumns() + " WHERE ";
            var splittedPrimaryKey = tableConfig.PrimaryKeyColumn.Split(new[] { ',', ';' }, StringSplitOptions.RemoveEmptyEntries);
            for (int i = 0; i < splittedPrimaryKey.Length; i++)
            {
                sql += $"{(i == 0 ? string.Empty : " AND ")}[{splittedPrimaryKey[i]}] = @{splittedPrimaryKey[i]}";
            }
            return sql;
        }
            ...
            TableConfig tableConfig)
        {
            ...
            if (!string.IsNullOrWhiteSpace(tableConfig.Criteria))
            {
                query += $" WHERE {tableConfig.Criteria}";
            }
            return query;
        }
        ...
        private string BuildCountSql(
            TableConfig tableConfig)
        {
            var query = $"SELECT COUNT(*) FROM [{tableConfig.Schema}].[{tableConfig.Name}]";
            if (!string.IsNullOrWhiteSpace(tableConfig.Criteria))
            {
                query += $" WHERE {tableConfig.Criteria}";
            }
